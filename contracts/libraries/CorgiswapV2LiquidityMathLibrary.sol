...
import "./SafeMath.sol";
import "./CorgiswapV2Library.sol";

// library containing some math for dealing with the liquidity shares of a pair, e.g. computing their exact value
// in terms of the underlying tokens
library CorgiswapV2LiquidityMathLibrary {
    ...
    function getReservesAfterArbitrage(
        ...
        (reserveA, reserveB) = CorgiswapV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ...
        require(
            ...
            "CorgiswapV2ArbitrageLibrary: ZERO_PAIR_RESERVES"
        );
        ...
        if (aToB) {
            uint256 amountOut = CorgiswapV2Library.getAmountOut(
                amountIn,
                reserveA,
                reserveB
            );
    ...
    function getLiquidityValue(
        ...
        (uint256 reservesA, uint256 reservesB) = CorgiswapV2Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
        ICorgiswapV2Pair pair = ICorgiswapV2Pair(
            CorgiswapV2Library.pairFor(factory, tokenA, tokenB)
        );
        bool feeOn = ICorgiswapV2Factory(factory).feeTo() != address(0);
